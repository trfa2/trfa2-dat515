// Package test provides utilities for generating test names and printing stack traces.
package test
...
func Name(name string, fields []string, values ...any) string {
	...
	if name != "" {
		b.WriteString(name)
		sep = "/" // set separator to / after the name
	}
	for i, f := range fields {
		...
		sep = "/" // set separator to / after the first field
	}
	return b.String()
}
